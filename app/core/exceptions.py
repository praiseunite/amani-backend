...
def create_error_response(
    status_code: int, message: str, error_code: str = None, details: dict = None, path: str = None
) -> JSONResponse:
    """
    ...
    """
    error_response = {
        "error": {"code": error_code or "ERROR", "message": message, "status": status_code}
    }
...
async def validation_error_handler(
    ...
    logger.warning(
        "Validation error",
        extra={"path": str(request.url), "method": request.method, "errors": errors},
    )
...
async def sqlalchemy_error_handler(request: Request, exc: SQLAlchemyError) -> JSONResponse:
    ...
    return create_error_response(
        status_code=status_code, message=message, error_code=error_code, path=str(request.url.path)
    )
