            ...
            if request.url.scheme != "https":
                ...
                if forwarded_proto != "https":
                    ...
                    return RedirectResponse(
                        url=str(url), status_code=status.HTTP_307_TEMPORARY_REDIRECT
                    )
...
class KYCEnforcementMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if request.method == "POST" and any(
            path.startswith(route) for route in self.PROTECTED_TRANSACTION_ROUTES
        ):
                ...
                try:
                    ...
                    try:
                        ...
                        stmt = select(Kyc).where(
                            Kyc.user_id == user.id, Kyc.status == KycStatus.APPROVED
                        )
                        ...
                        if kyc is None:
                            logger.warning(
                                f"User {user.id} attempted transaction without approved KYC"
                            )
                            return JSONResponse(
