...
def get_supabase_client() -> Client:
    ...
    if _supabase_client is None:
        if not settings.SUPABASE_URL or not settings.SUPABASE_KEY:
            logger.warning(
                "Supabase credentials not configured. Magic link authentication will not work."
            )
...
async def send_magic_link(email: str, redirect_url: Optional[str] = None) -> dict:
    ...
    try:
        ...
        response = supabase.auth.sign_in_with_otp(
            {"email": email, "options": options if redirect_url else {}}
        )
...
async def verify_magic_link_token(token: str) -> dict:
    ...
    try:
        ...
        # Verify the token
        response = supabase.auth.verify_otp({"token": token, "type": "magiclink"})
