...
async def create_milestone(
    ...
    logger.info(f"Milestone created: {new_milestone.id} for project {project.id} by user {current_user.email}")
...
async def list_milestones(
    ...
    status_filter: Optional[MilestoneStatus] = Query(None, description="Filter by milestone status"),
...
async def submit_milestone(
    ...
    if project.seller_id != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only the seller can submit milestones"
        )
...
async def approve_milestone(
    ...
    if project.buyer_id != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only the buyer can approve milestones"
        )
