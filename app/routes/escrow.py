...
async def hold_escrow_funds(
    ...
    result = await db.execute(
        select(Project).where(Project.id == escrow_data.project_id)
    )
    ...
    if project.buyer_id != current_user.id:
        raise HTTPException(
            ...
            detail="Only the buyer can hold funds in escrow"
        )
...
async def release_escrow_funds(
    ...
    # Fetch milestone
    result = await db.execute(
        select(Milestone).where(Milestone.id == release_data.milestone_id)
    )
    ...
    if milestone.is_paid:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Milestone has already been paid"
        )
    ...
    if project.buyer_id != current_user.id and not current_user.is_superuser:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only the buyer can release escrow funds"
        )
    ...
    try:
        ...
        seller_result = await db.execute(
            select(User).where(User.id == project.seller_id)
        )
        ...
        if not seller:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Seller not found for this project"
            )
