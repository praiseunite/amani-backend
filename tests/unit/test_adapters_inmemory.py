...
class TestInMemoryWalletRegistry:
    ...
    async def test_get_by_provider_wallet_not_found(self, registry):
        ...
        found = await registry.get_by_provider_wallet(uuid4(), WalletProvider.FINCRA, "nonexistent")
...
class TestInMemoryUserRepository:
    ...
    async def test_multiple_users(self, repository):
        ...
        users = [User(external_id=f"ext_{i}", email=f"user{i}@example.com") for i in range(5)]
...
class TestInMemoryAudit:
    ...
    async def test_events_ordered_chronologically(self, audit):
        ...
        await audit.record(
            user_id=user_id, action="first", resource_type="test", resource_id="1", details={}
        )
        await audit.record(
            user_id=user_id, action="second", resource_type="test", resource_id="2", details={}
        )
        await audit.record(
            user_id=user_id, action="third", resource_type="test", resource_id="3", details={}
        )
